...
public class DefaultMessageStore implements MessageStore {
    ...
    public boolean appendToCommitLog(long startOffset, byte[] data) {
        ...
        //向pageCache中写入数据  底层调用的 fileChannel.write
        boolean result = this.commitLog.appendData(startOffset, data);
        if (result) {
            //写成功 则环境 reput线程  构建逻辑消费队列
    ...
    class ReputMessageService extends ServiceThread {
        ...
        private void doReput() {
            for (boolean doNext = true; this.isCommitLogAvailable() && doNext; ) {
                ...
                //异步构建 consummerQueue 和index
                //reputFromOffset 构建过 consummerQueue/index的进度
                //result 存储的是已经刷过盘的 commitLog的进度 - reputFromOffset 就是可以构建consummerQueue/index的message
                        ...
                        for (int readSize = 0; readSize < result.getSize() && doNext; ) {
                            ...
                            if (dispatchRequest.isSuccess()) {
                                if (size > 0) {
                                    ...
                                    //构建完成后  判断是否要通知长轮询中等待的额请求
                                    if (BrokerRole.SLAVE != DefaultMessageStore.this.getMessageStoreConfig().getBrokerRole()
                                            && DefaultMessageStore.this.brokerConfig.isLongPollingEnable()) {
                                        //通知在 pullRequestHoldService的table中 等待的长轮询请求  数据来了 可以 重新发起
