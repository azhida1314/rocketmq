...
public class TopicConfigManager extends ConfigManager {
     ...
     *
     * @param topic                       主题
     * @param defaultTopic                默认topic TBW102
     * @param remoteAddress               broker地址
     * @param clientDefaultTopicQueueNums 对列大小
     * @param topicSysFlag                topic系统属性
     * @return
     */
    public TopicConfig createTopicInSendMessageMethod(final String topic, final String defaultTopic,
        ...
        try {
            if (this.lockTopicConfigTable.tryLock(LOCK_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                try {
                    ...
                    if (topicConfig != null)
                        return topicConfig;
                    //如过对应的topic不存在  且又打开了自动创建的开关  会根据默认topic的config来生成新的topic的config
        ...
        //如果进行了创建  把topic的路由信息注册到 nameserver  并且也会同步到 slaver 上
        if (createNew) {
