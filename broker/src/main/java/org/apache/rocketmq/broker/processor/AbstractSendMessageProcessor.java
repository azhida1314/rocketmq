...
public abstract class AbstractSendMessageProcessor implements NettyRequestProcessor {
     ...
     * 主要是检查
     * 1、topic的权限 是否是禁xie
     * 2、topic的路由信息存不存在 根据是否开启自动创建主题 决定不存在时怎么处理
     * 3、对应topic的发送队列是不是合法的
     *
     * @param ctx
     * @param requestHeader
     * @param response
    ...
    protected RemotingCommand msgCheck(final ChannelHandlerContext ctx,
        ...
        //topic的权限  可以禁读和禁写 在运维rocketMq时比较有用  顺序消息的扩容和缩容
        ...
        //不能和系统默认的topic冲突  MixAll.AUTO_CREATE_TOPIC_KEY_TOPIC TBW102
